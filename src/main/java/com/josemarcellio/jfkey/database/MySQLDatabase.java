...
public class MySQLDatabase implements DatabaseAPI {
    ...
    public MySQLDatabase(JFKey plugin) {
        this.plugin = plugin;
    }
    ...
    public void setup() {
        ...
        hikariConfig.setJdbcUrl("jdbc:mysql://" + plugin.getConfig().getString("mysql.address") + ":" + plugin.getConfig().getInt("mysql.port") + "/" + plugin.getConfig().getString("mysql.database"));
        ...
        hikariConfig.setMinimumIdle(plugin.getConfig().getInt("mysql.pool-settings.minimum-idle"));
        hikariConfig.setMaximumPoolSize(plugin.getConfig().getInt("mysql.pool-settings.maximum-pool-size"));
        hikariConfig.setMaxLifetime(plugin.getConfig().getLong("mysql.pool-settings.maximum-lifetime"));
        hikariConfig.setConnectionTimeout(plugin.getConfig().getLong("mysql.pool-settings.connection-timeout"));
        hikariConfig.setKeepaliveTime(plugin.getConfig().getLong("mysql.pool-settings.keepalive-time"));
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            plugin.getLogger().severe("Error creating josefkey_database table: " + e.getMessage());
        }
    }
