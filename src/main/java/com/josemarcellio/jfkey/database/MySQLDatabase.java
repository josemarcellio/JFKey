...
public class MySQLDatabase implements DatabaseAPI {
    ...
    public void setup() {
        ...
        hikariConfig.setMinimumIdle(
                plugin.getConfig().getInt("mysql.pool-settings.minimum-idle"));
        hikariConfig.setMaximumPoolSize(
                plugin.getConfig().getInt("mysql.pool-settings.maximum-pool-size"));
        hikariConfig.setMaxLifetime(
                plugin.getConfig().getLong("mysql.pool-settings.maximum-lifetime"));
        hikariConfig.setConnectionTimeout(
                plugin.getConfig().getLong("mysql.pool-settings.connection-timeout"));
        hikariConfig.setKeepaliveTime(
                plugin.getConfig().getLong("mysql.pool-settings.keepalive-time"));
        dataSource = new HikariDataSource(hikariConfig);
        try (Connection conn = dataSource.getConnection()) {
            PreparedStatement stmt = conn.prepareStatement(
                    "CREATE TABLE IF NOT EXISTS josefkey_database (player_id VARCHAR(36) " +
                            "NOT NULL, player_name VARCHAR(255) NOT NULL, command VARCHAR(255) " +
                            "NOT NULL, PRIMARY KEY (player_id))");
            stmt.executeUpdate();
    ...
    public void setCommand(UUID playerId, Player player, String command) {
        try (Connection conn = dataSource.getConnection()) {
            PreparedStatement stmt = conn.prepareStatement(
                    "INSERT INTO josefkey_database (player_id, player_name, command) " +
                            "VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE player_name = ?, command = ?");
            ...
            plugin.getLogger().severe(
                    "Error saving player command to MySQL: " + e.getMessage());
        }
    }
    ...
    public String getCommand(UUID playerId) {
        try (Connection conn = dataSource.getConnection()) {
            ...
            plugin.getLogger().severe(
                    "Error load player command from MySQL: " + e.getMessage());
            return "no_command_set";
        }
    }
}